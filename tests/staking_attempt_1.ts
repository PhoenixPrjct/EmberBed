...
const admin_json_path = require('path').join(__dirname, "../phoenixdev.json")
...
describe("staking_attempt_1", async () => {
  ...
  const RewTok = new PublicKey("F1rEZqWk1caUdaCwyHMWhxv5ouuzPW8sgefwBhzdhGaw")

  // Admin Variables
  // const RewTok = new PublicKey("REWLYBz1rZFS2ErKB5LEdXTYChUXBAvF2zjbizsAsjM");
  // new PublicKey("REWTvQ7zqtfoedwsPGCX9TF59HvAoM76LobtzmPPpko");
  ...
  const rewardSymbol: string = "$F1RE";
  const collectionName: string = "EYES"
  ...
  before(async () => {
    ...
    ratePerDay = 1000
    //* Staking
    const mintAddress: PublicKey = new PublicKey("EPhYDdEFQouxp4AtXUhK93qdWKpsVD5MVrWJV9tJwo7w")
    ...
    console.log("generated nft token address: ", nftTokenAddress.toBase58());

    userAta = await getAssociatedTokenAddress(RewTok, UserWallet.publicKey)
    ...
    console.log("Auth PDA", delegatedAuthPda.toBase58());
  ...
  it("Allows Manager to Deposit to Program Owned Account", async () => {
    ...
    const tx2 = await program.methods.depositToRewardAta(bumpState, bumpToken, amount).accounts({
      statePda: statePDA,
      tokenPoa: rewardWallet.address,
      mint: RewTok,
      funder: AdminWallet.publicKey,
  ...
  it(" Allows Manager to withdraw tokens", async () => {
    ...
    const closeAta = true;
    const tx = await program.methods.managerWithdrawal(bumpState, closeAta, amount).accounts({
  ...
  it("Stakes NFT", async () => {
    ...
    let stakeStatus = await program.account.userStakeInfo.fetch(stakeStatusPda)
  ...
  it("Unstakes NFT", async () => {
    ...
    // const stakeStatusPdaV2 = new PublicKey("BQm4FGTz1sWY6mmmTy9UBK4kYWofrV2Ebxg4BSJtUSRz")
