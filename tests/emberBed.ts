...
describe("EmberBed", async () => {
  ...
  before(async () => {
    ...
    // userAta = await getOrCreateAssociatedTokenAccount(connection, UserWallet, RewTok, UserWallet.publicKey)
  ...
  it("Stakes NFT", async () => {
    ...
    let stakeStateExists = await program.account.userStakeInfo.getAccountInfo(stakeStatusPda)
    let stakeStatus = stakeStateExists ? await program.account.userStakeInfo.fetch(stakeStatusPda) : { stakeState: { unstaked: {} } };
